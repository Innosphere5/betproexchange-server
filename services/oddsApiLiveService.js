...
class OddsApiLiveService {
    ...
    handleMessage(msg) {
        ...
        switch (type) {
            case 'login_ok':
                this.reconnectAttempts = 0;
                console.log('[OddsApiLive] ✅ Received login_ok. Triggering bootstrap...');
    ...
    async bootstrapUpcomingFixtures() {
        ...
        try {
            ...
            try {
                ...
                console.warn('[OddsApiLive] ⚠️ Could not fetch live fixtures:', err.message);
            }
            ...
            for (const fixture of liveFixtures) {
                ...
                const matchId = await this.linkEventToMatch((fixture.fixtureId || fixture.id));
                if (matchId) {
                    totalLinked++;
                    try {
                        const oddsData = await oddsApiRest.getFixtureOdds((fixture.fixtureId || fixture.id), BOOKMAKERS);
                        if (oddsData) {
            ...
            const linkedMatchIds = new Set(Array.from(this.eventToMatchId.values()).map(String));
            const unlinkedActive = activeMatches.filter(m => !linkedMatchIds.has(String(m.matchId)));
            if (unlinkedActive.length > 0) {
                console.warn(`[OddsApiLive] ⚠️ ${unlinkedActive.length} match(es) NOT found on odds API (no odds coverage):`,
                    unlinkedActive.map(m => `${m.teamA} v ${m.teamB} [${m.status}]`).join(', '));
            }
            console.log(`[OddsApiLive] 📅 Bootstrap complete: ${totalLinked} fixtures linked.`);
        } catch (err) {
    ...
    storeFixtureMetadata(fixture) {
        ...
        const sport = fixture.sport?.sportName || fixture.sport_key || fixture.sport;
        const commenceTime = fixture.startTime ? new Date(fixture.startTime * 1000).toISOString() : fixture.commence_time;
    ...
    async linkEventToMatch(eventId) {
        ...
        try {
            ...
            for (const match of matches) {
                ...
                const homeContains = (dbHome.includes(apiHome) || apiHome.includes(dbHome)) && dbHome.length >= 3 && apiHome.length >= 3;
                const awayContains = (dbAway.includes(apiAway) || apiAway.includes(dbAway)) && dbAway.length >= 3 && apiAway.length >= 3;
                const homeContainsSwap = (dbHome.includes(apiAway) || apiAway.includes(dbHome)) && dbHome.length >= 3 && apiAway.length >= 3;
                const awayContainsSwap = (dbAway.includes(apiHome) || apiHome.includes(dbAway)) && dbAway.length >= 3 && apiHome.length >= 3;

                const teamsMatch = (homeContains && awayContains) || (homeContainsSwap && awayContainsSwap);

                if (teamsMatch) {
                    const timeDiff = apiTime > 0 ? Math.abs(dbTime - apiTime) / (1000 * 60 * 60) : 0;
                    if (timeDiff <= 12 || apiTime === 0) {
                        this.eventToMatchId.set(eventId, match.matchId);
                        console.log(`[OddsApiLive] 🔗 Linked (fuzzy) ${metadata.home} v ${metadata.away} → match ${match.matchId}`);
                        return match.matchId;
                    }
                }
            }
    ...
    normalize(name) {
        ...
        n = n.replace(/\bwomen\b/g, '').replace(/\bteam\b/g, '')
        ...
        const aliases = {
            ...
            'lucknowsupergiants': 'lsg', 'lucknow': 'lsg',
            'sunrisershyderabad': 'srh', 'sunrisers': 'srh',
            'mumbaiindians': 'mi',
            ...
            'punjabkings': 'pbks', 'kingsxi': 'pbks',
            // International Teams
            'india': 'india', 'bharat': 'india',
            'australia': 'australia', 'aussies': 'australia',
            'england': 'england',
            'southafrica': 'southafrica', 'proteas': 'southafrica',
            'newzealand': 'newzealand', 'blackcaps': 'newzealand',
            'pakistan': 'pakistan',
            'srilanka': 'srilanka',
            'westindies': 'westindies', 'windies': 'westindies',
            ...
            'peshaarzalmi': 'peshawarzalmi', 'peshawarzalmi': 'peshawarzalmi',
    ...
    async handleOddsData(msg) {
        ...
        console.log(`[OddsApiLive] ⚡ WebSocket odds received for event ${eventId} (match ${matchId})`);
    ...
    async processOddsForEvent(eventObj) {
        ...
        let bestHomePrice = 0;
        let bestAwayPrice = 0;
        let homeBack = 0, homeBackVol = '0', homeLay = 0, homeLayVol = '0';
        let awayBack = 0, awayBackVol = '0', awayLay = 0, awayLayVol = '0';
        let usedBookie = '';
        ...
        for (const bookieKey of allowedKeys) {
            ...
            const isExchange = bookieKey.toLowerCase().includes('exchange') || bookieKey.toLowerCase().includes('ex');
            ...
            if (Array.isArray(outcomes)) {
                ...
                const quoteList = Object.values(outcomes).filter(q => q && q.active !== false);
            ...
            if (homeOutcome && awayOutcome && homeOutcome.price && awayOutcome.price) {
                ...
                let changedAt = homeOutcome.changedAt || homeOutcome.changed_at || homeOutcome.updatedAt || homeOutcome.updated_at;
                if (changedAt) {
                    const parsedTime = typeof changedAt === 'string' ? new Date(changedAt).getTime() : Number(changedAt);
                    if (!isNaN(parsedTime) && parsedTime > 0) {
                        ...
                        if (quoteAgeMs > maxAgeMs) {
                            console.log(`[OddsApiLive] ⚠️ Skipping bookmaker '${bookieKey}' for event ${eventId} due to stale quotes (age: ${Math.round(quoteAgeMs / 1000 / 60)} minutes)`);
                            continue;
                        }
                    }
                }
                ...
                if (isExchange && homeOutcome.meta) {
                    homeBack = homeOutcome.meta.availableToBack?.[0]?.price || homeOutcome.price || 0;
                    homeBackVol = homeOutcome.meta.availableToBack?.[0]?.size !== undefined ? Math.round(homeOutcome.meta.availableToBack[0].size).toString() : '100';
                    homeLay = homeOutcome.meta.availableToLay?.[0]?.price || Number((homeBack + 0.02).toFixed(2));
                    homeLayVol = homeOutcome.meta.availableToLay?.[0]?.size !== undefined ? Math.round(homeOutcome.meta.availableToLay[0].size).toString() : '100';
                } else {
                    homeBack = homeOutcome.price;
                    homeBackVol = homeOutcome.limit !== undefined ? Math.round(homeOutcome.limit).toString() : '500';
                ...
                if (isExchange && awayOutcome.meta) {
                    awayBack = awayOutcome.meta.availableToBack?.[0]?.price || awayOutcome.price || 0;
                    awayBackVol = awayOutcome.meta.availableToBack?.[0]?.size !== undefined ? Math.round(awayOutcome.meta.availableToBack[0].size).toString() : '100';
                    awayLay = awayOutcome.meta.availableToLay?.[0]?.price || Number((awayBack + 0.02).toFixed(2));
                    awayLayVol = awayOutcome.meta.availableToLay?.[0]?.size !== undefined ? Math.round(awayOutcome.meta.availableToLay[0].size).toString() : '100';
                } else {
                    awayBack = awayOutcome.price;
                    awayBackVol = awayOutcome.limit !== undefined ? Math.round(awayOutcome.limit).toString() : '500';
        ...
        if (this.deletedGraceTimers.has(eventId)) {
            ...
            console.log(`[OddsApiLive] ♻️ Cancelled grace-period suspension for event ${eventId} — fresh odds received`);
        }
        ...
        if (this.io) {
            ...
            console.log(`[OddsApiLive] 📡 Emitting odds update to UI for match ${matchId} (${dbTeamAName} v ${dbTeamBName}) | Bookmaker: ${usedBookie} | Odds: A(${teamABack}/${teamALay}) B(${teamBBack}/${teamBLay})`);
    ...
    async handleDeleted(msg) {
        ...
        console.log(`[OddsApiLive] ⏳ Event ${eventId} (match ${matchId}) received 'deleted' — starting 2-min grace period`);

        const timer = setTimeout(async () => {
            this.deletedGraceTimers.delete(eventId);
            console.log(`[OddsApiLive] ⛔ Grace period expired for event ${eventId} (match ${matchId}) — suspending market`);
            if (this.io) {
                this.io.emit('market_odds_update', { matchId: String(matchId), marketStatus: 'SUSPENDED' });
            }
    ...
    async pollLinkedFixtures() {
        try {
            ...
            const linkedMatchIds = new Set(Array.from(this.eventToMatchId.values()).map(String));
            const unlinkedLive = liveMatches.filter(m => !linkedMatchIds.has(String(m.matchId)));
            ...
            if (unlinkedLive.length > 0) {
                ...
                if (msSinceLastBootstrap > 5 * 60 * 1000) { // 5 minutes
                    this.lastBootstrapAttempt = Date.now();
                    console.log(`[OddsApiLive] 🔄 Found ${unlinkedLive.length} unlinked live match(es). Re-bootstrapping...`);
                    this.bootstrapUpcomingFixtures().catch(() => {});
                }
            }

            for (const [eventId, matchId] of this.eventToMatchId.entries()) {
                const isLive = liveMatches.some(m => String(m.matchId) === String(matchId));
                if (!isLive) continue;

                try {
                    console.log(`[OddsApiLive] 🔄 Polling REST live odds for event ${eventId} (match ${matchId})...`);
                    const oddsData = await oddsApiRest.getFixtureOdds(eventId, BOOKMAKERS);
                    if (oddsData) {
                        console.log(`[OddsApiLive] ✅ REST live odds successfully fetched for event ${eventId}`);
                        const dataArray = Array.isArray(oddsData) ? oddsData : [oddsData];
                        for (const odd of dataArray) {
                            await this.processOddsForEvent(odd);
                        }
                    ...
                    console.error(`[OddsApiLive] ❌ REST live poll failed for event ${eventId}:`, err.message);
                }
            }
        } catch (err) {}
    }
    ...
    async pollUpcomingFixtures() {
        try {
            ...
            for (const [eventId, matchId] of this.eventToMatchId.entries()) {
                ...
                try {
                    console.log(`[OddsApiLive] 🔄 Polling REST upcoming odds for event ${eventId} (match ${matchId})...`);
                    const oddsData = await oddsApiRest.getFixtureOdds(eventId, BOOKMAKERS);
                    if (oddsData) {
                        console.log(`[OddsApiLive] ✅ REST upcoming odds successfully fetched for event ${eventId}`);
                        const dataArray = Array.isArray(oddsData) ? oddsData : [oddsData];
                        for (const odd of dataArray) {
                            await this.processOddsForEvent(odd);
                        }
                    ...
                    console.error(`[OddsApiLive] ❌ REST upcoming poll failed for event ${eventId}:`, err.message);
                }
            }
