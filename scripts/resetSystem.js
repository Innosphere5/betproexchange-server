...
async function resetSystem() {
  try {
    ...
    // Reset SuperAdmin account balances to 1 Crore (₹10,000,000)
    const superadminRes = await User.updateMany(
      { role: 'superadmin' },
      { $set: { walletBalance: 10000000, credit: 0 } }
    );
    console.log(`Reset ${superadminRes.modifiedCount} superadmin account(s) wallet balance to 1 Crore (₹10,000,000).`);
